...
public class FilePathDemoTest_Cm {
    ...
    public List<DoorData> listFilesDirCmService(){
        ...
        String path = "D:\\idea-Work\\Localization\\cm\\src\\main\\java\\com\\fenet\\insurance";
    ...
    private List<DoorData> handle(File file) {
        ...
        String packageKey01 = "";
        if (packageKey.contains("controller")) {
             packageKey01 = packageKey.substring(0, packageKey.indexOf("\\controller\\"));
        }else if(packageKey.contains("servlet")) {
            packageKey01 = packageKey.substring(0, packageKey.indexOf("\\servlet\\"));
        }else{
            System.err.println("===--处理不了路径--==="+packageKey);
        }
            ...
            for(String key : result.keySet()){
               // System.out.println(myIndex +"---key : " + key  +" size : " + result.get(key).size());
                for (String method :result.get(key)){
                    ...
                    if (flag) {
                        data.setPackagePath(packageKey01);
                        data.setDoorController(packageKey);
                        flag = false ;
                    }else{
                        data.setPackagePath("");
                        data.setDoorController("");
